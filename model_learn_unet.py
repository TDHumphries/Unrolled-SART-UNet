...
# This is the LEARN algorithm with unet implement in every other 3 iterations
def LEARN(sino, geometry, dx, numpix, numits, is_training):
    current_x = tf.zeros([1, numpix[0], numpix[0]])
    for iteration in range(numits):
        # pdb.set_trace()
...
# Innovation arch based on 2-2 UNet with elu activation + he. Second try of adding elements of ResNet.
# + Conv2DTranspose + SE block + increase the number of feature outputs to (max)256 + fix the activation in se &
# residual
def cnn_layers(user_input, numpix, iteration, is_training=True, output_channels=1):
    with tf.variable_scope('CNN_layer{}'.format(iteration)):
        user_input = tf.expand_dims(user_input,-1)  # add channel dimension
        conv1_1 = tf.keras.layers.Conv2D(64, 7, padding='same', activation=tf.keras.layers.ELU(),
                                         kernel_initializer=tf.keras.initializers.he_normal(),
                                         kernel_regularizer=tf.keras.regularizers.l2(0.01),
                                         activity_regularizer=tf.keras.regularizers.l2(0.01),
                                         name='conv1_1')(user_input)
        pool1 = tf.keras.layers.MaxPool2D(pool_size=(2, 2), strides=(2, 2), name='pool1')(conv1_1)

        conv2_1 = tf.keras.layers.Conv2D(128, 3, padding='same', activation=tf.keras.layers.ELU(),
                                         kernel_initializer=tf.keras.initializers.he_normal(),
                                         kernel_regularizer=tf.keras.regularizers.l2(0.01),
                                         activity_regularizer=tf.keras.regularizers.l2(0.01),
                                         name='conv2_1')(pool1)
        conv2_2 = tf.keras.layers.Conv2D(128, 3, padding='same', activation=tf.keras.layers.ELU(),
                                         kernel_initializer=tf.keras.initializers.he_normal(),
                                         kernel_regularizer=tf.keras.regularizers.l2(0.01),
                                         activity_regularizer=tf.keras.regularizers.l2(0.01),
                                         name='conv2_2')(conv2_1)
        conv2_3 = tf.keras.layers.Conv2D(128, 1, padding='same', activation=None,
                                         kernel_initializer=tf.keras.initializers.he_normal(),
                                         kernel_regularizer=tf.keras.regularizers.l2(0.01),
                                         activity_regularizer=tf.keras.regularizers.l2(0.01),
                                         name='conv2_3')(pool1)

        se2_1 = tf.keras.layers.GlobalAveragePooling2D()(conv2_2)
        se2_2 = tf.keras.layers.Dense(128 // 16, activation=tf.keras.layers.ELU())(se2_1)
        se2_3 = tf.keras.layers.Dense(128, activation='sigmoid')(se2_2)
        se2_4 = tf.keras.layers.multiply([conv2_2, se2_3])

        res2_1 = tf.keras.layers.Add()([se2_4, conv2_3])
        res2_2 = tf.keras.layers.Activation(activation=tf.keras.layers.ELU())(res2_1)

        drop2 = tf.keras.layers.Dropout(0.5, name='drop2')(res2_2)

        pool2 = tf.keras.layers.MaxPool2D(pool_size=(2, 2), strides=(2, 2), name='pool2')(drop2)

        conv3_1 = tf.keras.layers.Conv2D(256, 3, padding='same', activation=tf.keras.layers.ELU(),
                                         kernel_initializer=tf.keras.initializers.he_normal(),
                                         kernel_regularizer=tf.keras.regularizers.l2(0.01),
                                         activity_regularizer=tf.keras.regularizers.l2(0.01),
                                         name='conv3_1')(pool2)
        conv3_2 = tf.keras.layers.Conv2D(256, 3, padding='same', activation=tf.keras.layers.ELU(),
                                         kernel_initializer=tf.keras.initializers.he_normal(),
                                         kernel_regularizer=tf.keras.regularizers.l2(0.01),
                                         activity_regularizer=tf.keras.regularizers.l2(0.01),
                                         name='conv3_2')(conv3_1)
        conv3_3 = tf.keras.layers.Conv2D(256, 1, padding='same', activation=None,
                                         kernel_initializer=tf.keras.initializers.he_normal(),
                                         kernel_regularizer=tf.keras.regularizers.l2(0.01),
                                         activity_regularizer=tf.keras.regularizers.l2(0.01),
                                         name='conv3_3')(pool2)

        se3_1 = tf.keras.layers.GlobalAveragePooling2D()(conv3_2)
        se3_2 = tf.keras.layers.Dense(256 // 16, activation=tf.keras.layers.ELU())(se3_1)
        se3_3 = tf.keras.layers.Dense(256, activation='sigmoid')(se3_2)
        se3_4 = tf.keras.layers.multiply([conv3_2, se3_3])

        res3_1 = tf.keras.layers.Add()([se3_4, conv3_3])
        res3_2 = tf.keras.layers.Activation(activation=tf.keras.layers.ELU())(res3_1)

        drop3 = tf.keras.layers.Dropout(0.5, name='drop3')(res3_2)

        up8 = tf.keras.layers.Conv2DTranspose(128, 2, padding='same', activation=tf.keras.layers.ELU(),
                                              kernel_initializer=tf.keras.initializers.he_normal(),
                                              kernel_regularizer=tf.keras.regularizers.l2(0.01),
                                              activity_regularizer=tf.keras.regularizers.l2(0.01),
                                              name='up8')(tf.keras.layers.UpSampling2D(size=(2, 2))(drop3))
        merge8 = tf.keras.layers.concatenate([drop2, up8], axis=3)

        conv8_1 = tf.keras.layers.Conv2D(128, 3, padding='same', activation=tf.keras.layers.ELU(),
                                         kernel_initializer=tf.keras.initializers.he_normal(),
                                         kernel_regularizer=tf.keras.regularizers.l2(0.01),
                                         activity_regularizer=tf.keras.regularizers.l2(0.01),
                                         name='conv8_1')(merge8)
        conv8_2 = tf.keras.layers.Conv2D(128, 3, padding='same', activation=tf.keras.layers.ELU(),
                                         kernel_initializer=tf.keras.initializers.he_normal(),
                                         kernel_regularizer=tf.keras.regularizers.l2(0.01),
                                         activity_regularizer=tf.keras.regularizers.l2(0.01),
                                         name='conv8_2')(conv8_1)
        conv8_3 = tf.keras.layers.Conv2D(128, 1, padding='same', activation=None,
                                         kernel_initializer=tf.keras.initializers.he_normal(),
                                         kernel_regularizer=tf.keras.regularizers.l2(0.01),
                                         activity_regularizer=tf.keras.regularizers.l2(0.01),
                                         name='conv8_3')(merge8)

        se8_1 = tf.keras.layers.GlobalAveragePooling2D()(conv8_2)
        se8_2 = tf.keras.layers.Dense(128 // 16, activation=tf.keras.layers.ELU())(se8_1)
        se8_3 = tf.keras.layers.Dense(128, activation='sigmoid')(se8_2)
        se8_4 = tf.keras.layers.multiply([conv8_2, se8_3])

        res8_1 = tf.keras.layers.Add()([se8_4, conv8_3])
        res8_2 = tf.keras.layers.Activation(activation=tf.keras.layers.ELU())(res8_1)
        drop8 = tf.keras.layers.Dropout(0.5, name='drop8')(res8_2)
        up9 = tf.keras.layers.Conv2DTranspose(64, 2, padding='same', activation=tf.keras.layers.ELU(),
                                              kernel_initializer=tf.keras.initializers.he_normal(),
                                              kernel_regularizer=tf.keras.regularizers.l2(0.01),
                                              activity_regularizer=tf.keras.regularizers.l2(0.01),
                                              name='up9')(tf.keras.layers.UpSampling2D(size=(2, 2))(drop8))
        merge9 = tf.keras.layers.concatenate([conv1_1, up9], axis=3)

        conv9_1 = tf.keras.layers.Conv2D(64, 3, padding='same', activation=tf.keras.layers.ELU(),
                                         kernel_initializer=tf.keras.initializers.he_normal(),
                                         kernel_regularizer=tf.keras.regularizers.l2(0.01),
                                         activity_regularizer=tf.keras.regularizers.l2(0.01),
                                         name='conv9_1')(merge9)
        conv9_2 = tf.keras.layers.Conv2D(64, 3, padding='same', activation=tf.keras.layers.ELU(),
                                         kernel_initializer=tf.keras.initializers.he_normal(),
                                         kernel_regularizer=tf.keras.regularizers.l2(0.01),
                                         activity_regularizer=tf.keras.regularizers.l2(0.01),
                                         name='conv9_2')(conv9_1)
        conv9_3 = tf.keras.layers.Conv2D(64, 1, padding='same', activation=None,
                                         kernel_initializer=tf.keras.initializers.he_normal(),
                                         kernel_regularizer=tf.keras.regularizers.l2(0.01),
                                         activity_regularizer=tf.keras.regularizers.l2(0.01),
                                         name='conv9_3')(merge9)

        se9_1 = tf.keras.layers.GlobalAveragePooling2D()(conv9_2)
        se9_2 = tf.keras.layers.Dense(64 // 16, activation=tf.keras.layers.ELU())(se9_1)
        se9_3 = tf.keras.layers.Dense(64, activation='sigmoid')(se9_2)
        se9_4 = tf.keras.layers.multiply([conv9_2, se9_3])

        res9_1 = tf.keras.layers.Add()([se9_4, conv9_3])
        res9_2 = tf.keras.layers.Activation(activation=tf.keras.layers.ELU())(res9_1)
        drop9 = tf.keras.layers.Dropout(0.5, name='drop9')(res9_2)

        # conv9_4 = tf.keras.layers.Conv2D(2, 3, padding='same', activation=tf.keras.layers.ELU(),
        #                                  kernel_initializer=tf.keras.initializers.he_normal(),
        #                                  name='conv9_4')(drop9)
        # conv10 = tf.keras.layers.Conv2D(1, 1, activation='sigmoid', name='conv10')(conv9_4)
        conv10 = tf.keras.layers.Conv2D(1, 1, padding='same', activation=tf.keras.layers.ELU(),
                                        kernel_initializer=tf.keras.initializers.he_normal(),
                                        kernel_regularizer=tf.keras.regularizers.l2(0.01),
                                        activity_regularizer=tf.keras.regularizers.l2(0.01),
                                        name='conv10')(drop9)
...
class ReconNet(object):
    ...
    def __init__(self, sess, numpix,dx,numbin,numtheta,numits,
        ...
        self.cnn_model_name = "model_learn_unet_1"
        ...
        regexp = re.compile(r'LEARN_layer\d{1,2}/stepsize:\d{1,2}')
        ...
        with tf.control_dependencies(update_ops):
            ...
            # self.train_op = optimizer.apply_gradients(self.gvs)
